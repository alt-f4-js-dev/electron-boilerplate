...
function addMDOPages(element, container) {
  let value = element.value;
  let con = document.getElementById(container);
  let childCount = con.childElementCount;
  if (con.id == "SiteDetails_Container") {
    let childCount = con.childElementCount;
    while (childCount > 1) {
      con.removeChild(con.lastElementChild);
      childCount = document.getElementById(container).childElementCount;
    }
...
function SubmInfo_PropInvoAnyDevBel_Change(element) {
  // let switchButton = document.querySelector("[switch-id='SubmInfo_PropInvoAnyDevBel_Yes10']")
  let switchButtons = document.querySelectorAll("[swAttr]");

  for (let b of switchButtons) {
    let select = b.parentNode.parentNode.querySelector("cn2-select");
    if (b.checked) {
...
function calculatePlotRatio(
  ...
  document.getElementById(grossPlotRatioPrefix + suffix).value = (
    parseFloat(
      checkIfEmpty(document.getElementById(gfaPrefix + suffix).value)
    ) /
    parseFloat(
      checkIfEmpty(document.getElementById(siteAreaPrefix + suffix).value)
    )
  ).toFixed(2);
  document.getElementById(grossPlotRatioPrefix + suffix).value = parseFloat(
    checkNaN(document.getElementById(grossPlotRatioPrefix + suffix).value)
  ).toFixed(2);
}
