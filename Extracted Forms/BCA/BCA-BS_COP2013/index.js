...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  //     a.shadowRoot.querySelector("input").style.textAlign = "center";
  // }
  for (let a of document.getElementById("page9").querySelectorAll("td")) {
    a.setAttribute("page-owner", "page9");
  }
  ...
  // for validating if all mandatory fields in page7 is filled up
  for (let a of document
...
function appTypeChanged(type) {
  ...
  if (types.includes(type)) {
    ...
    let formName = ["BCA-BS01_COP2013", "BCA-BS02_COP2013", "BCA-BS03_COP2013"];
...
function showByAppType(el) {
  ...
  if (appTypeVal == "bs01") {
    ...
    for (let a of document.querySelectorAll("[app-type-bs01]")) {
      if (a.querySelector("[page4-totalbds]"))
        a.querySelector("[page4-totalbds]").value = "0";
      if (a.hasAttribute("hidden")) a.removeAttribute("hidden");
    ...
    document.getElementById("grandTotal").innerHTML = "GRAND TOTAL (A + B + C + D + E) ";
    ...
    document.getElementById("grandTotal").innerHTML = "GRAND TOTAL (A + C + D + E) ";
  ...
  let page5 = document.getElementById("page7");
...
// displaying the minimum buildable design score
function minimumBuildDesignScore() {
  ...
  if (newWorkVal != 0 && existingVal != 0) {
    document.querySelector("[total-bds-round]").value = !isNaN(Math.round(total)) ? Math.round(total) : "0";
  } else if (newWorkVal != 0) {
    document.querySelector("[total-bds-round]").value = !isNaN(Math.round(total)) ? Math.round(total) : "0";
  } else if (existingVal != 0) {
    let existingMax = 0
    existingMax = total
    document.querySelector("[total-bds-round]").value = !isNaN(Math.round(existingMax)) ? Math.round(existingMax) : "0";
  }

  let minScoreValue = document.querySelector("[total-bds-round]").value;

  if (newWorkVal != 0) {
    for (let a of document.querySelectorAll('[name="devRadio"]')) {
      //if radio is checked
      if (a.checked === true) {
        if (a.id == "CalcOfOverBuil_ProjDetl_Deve10") {
          if (minScoreValue != 0 && minScoreValue != "") {
            document.querySelector("[total-bds-round]").value =
              parseInt(minScoreValue) + parseInt(8);
          } else {
            document.querySelector("[total-bds-round]").value = minScoreValue;
          }
        ...
        } else if (a.id == "CalcOfOverBuil_ProjDetl_Deve30") {
          if (minScoreValue != 0 && minScoreValue != "") {
            document.querySelector("[total-bds-round]").value =
              parseInt(minScoreValue) + parseInt(11);
          } else {
            document.querySelector("[total-bds-round]").value = minScoreValue;
          }
        } else if (a.id == "CalcOfOverBuil_ProjDetl_Deve40") {
          if (minScoreValue != 0 && minScoreValue != "") {
            document.querySelector("[total-bds-round]").value =
              parseInt(minScoreValue) + parseInt(3);
          } else {
            document.querySelector("[total-bds-round]").value = minScoreValue;
          }
        }
...
function refreshPage() {
  ...
  let firstPageNo =
    [
      ...
      .map((r) => r.getAttribute("page-number"))
      .map((r) => parseInt(r))
      .sort()
      .pop() + 1;
  let temp = firstPage;
  ...
  for (let a of document
    ...
    a.setAttribute("page-number", firstPageNo);
...
function distributeTotal(el, masterBlock) {
  ...
  let page8_1 = document.getElementById("page8_1");
  let page = findBlock(document.getElementById(el.id));
  let childFieldNew = page8_1
    .querySelector("[blockchildtr='" + masterBlock + "']")
    .querySelector("td");
  ...
  for (let a of page8_1.querySelectorAll(
    "[blockchildtr='" + masterBlock + "']"
  )) {
...
function overAllTotal() {
  let page8_1 = document.getElementById("page8_1");
  let totalAreaField = page8_1.querySelector("[total-area]");
  let totalPercentField = page8_1.querySelector("[total-percent]");
  let totalABDSField = page8_1.querySelector("[total-abds]");
  let totalABDSRoundedField = page8_1.querySelector("[total-abds-rounded]");
  //let totalBDSRoundedField = page8_1.querySelector("[total-bds-round]");
  let totalBDSOnPage4 = document.querySelectorAll("[page4-totalBDS]");
  let totalArea = [...page8_1.querySelectorAll("[floor]")]
    .filter((a) => !isNaN(parseFloat(a.value)))
    .map((a) => parseFloat(a.value))
    .reduce((a, b) => a + b, 0)
    .toFixed(2);
  let totalABDS = [...page8_1.querySelectorAll("[abds]")]
  ...
  for (let a of page8_1.querySelectorAll("[floor]")) {
    let total = page8_1.querySelector("[total-area]").value;
  ...
  for (let a of page8_1.querySelectorAll("[abds]")) {
  ...
  page8_1.querySelector("[total-abds]").value = [...page8_1.querySelectorAll("[abds]")]
    .map((el) => parseFloat(el.value))
    .reduce((a, b) => a + b)
    .toFixed(2);

  page8_1.querySelector("[total-abds-rounded]").value = Math.round(
    [...page8_1.querySelectorAll("[abds]")]
      .map((el) => parseFloat(el.value))
      .reduce((a, b) => a + b)
  );
...
function BDS_SS_A3(el) {
  ...
  if (sum <= 45) {
    ...
    totalField.setAttribute("raw-value", parseFloat(45).toFixed(2));
    !isNaN(parseFloat(45))
...
function BDS_SS(el) {
  ...
  let sum = formattedA1 + formattedA3;
  let totalSum = 0;
  if (sum > 50) {
    totalSum = 50;
  } else {
...
function BDS_WS_B3(el) {
  ...
  if (sum <= 40) {
    ...
    totalField.setAttribute("raw-value", parseFloat(40).toFixed(2));
    !isNaN(parseFloat(40))
...
function BDS_Total(el) {
  ...
  for (let a of fields) {
    !isNaN(parseFloat(validateTotal))
      ? (a.value = parseFloat(validateTotal).toFixed(2))
      : (a.value = "");
  }

  page.querySelector(
    "[prefix='CalcOfOverBuil_CompOfBuilDesi_StruSystC_GranTota']"
  ).value = !isNaN(parseFloat(validateTotal)) ? parseFloat(validateTotal).toFixed(2) : "";
